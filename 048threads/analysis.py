...
if __name__ == '__main__':
    ...
    def summarize_threads(df):
        ...
        # df_summ['l2miss'] = (dfg['L2D_CACHE_REFILL'].sum() - dfg['L2D_SWAP_DM'].sum() - dfg['L2D_CACHE_MIBMCH_PRF'].sum())/ rep_meas
    ...
    print(len(df_meas_sc))
    print(len(df_meas_nosc))
    ...
    print(df_meas[~df_meas['matrix'].isin(df_pred['matrix'])])
    ...
    # df = df.loc[df['nnz'] * 12 + (16 + 1 * 8) * df['rows'] > 32 * 1024 * 1024] # this filter
    ...
    # ax.legend(['MAPE SC (5 L2 Ways)', 'MAPE NO SC'])
    ...
    for i in range(0, int((rmax-rmin) / 2) + 1):
        # ax.annotate(str(count[i]), (i - 0.18, 0.8), color='darkslategray', fontsize=10.0, rotation=90)
        # ax.annotate('min=' + str(min_size[i]) + 'MiB ' + 'avg=' +  str(avg_size[i]) + 'MiB ' + 'count=' + str(count[i]), (i - 0.18, 0.8), color='darkslategray', fontsize=10.0, rotation=90)
    ...
    df_bad_1 = df.loc[((df['l2red_meas'] > 0) & (df['l2red_pred'] < 0))]
    df_bad_2 = df.loc[((df['l2red_meas'] < 0) & (df['l2red_pred'] > 0))]
    df_bad_1 = df_bad_1.loc[abs(df_bad_1['l2red_meas'] / df_bad_1['l2miss_nosc_meas']) > 0.01]
    df_bad_2 = df_bad_2.loc[abs(df_bad_2['l2red_meas'] / df_bad_2['l2miss_nosc_meas']) > 0.01]
    # df_bad = df.loc[df['l2red_pred'] < 0]
    # print('fail:')
    # print(df_bad_1[['matrix', 'nnz_per_row', 'l2miss_nosc_meas', 'l2miss_sc_meas', 'l2red_meas', 'l2red_pred']])
    # print(df_bad_1[['matrix', 'nnz_per_row', 'l2miss_nosc_meas', 'l2miss_sc_meas', 'l2red_meas', 'l2red_pred']])
    # print(df_bad_2[['matrix', 'nnz_per_row', 'l2miss_nosc_meas', 'l2miss_sc_meas', 'l2red_meas', 'l2red_pred']])
    # print(df_bad_1)
    # print(df_bad_2)
    ...
    # df = df.loc[df['l2ape_nosc'] <= 10]
    print('num matrices:', len(df))

    df1 = df.loc[df['l2ape_nosc'] > 10]
    print('ape no sc > 10')
    print(df1[['matrix', 'nnz', 'nnz_per_row', 'rows', 'l2miss_nosc_meas', 'l2miss_nosc_pred', 'l2ape_nosc', 'variance', 'nnz_per_row_var_coeff']])

    df1 = df.loc[df['l2ape_sc'] > 10]
    print('ape sc > 10')
    print(df1[['matrix', 'nnz', 'nnz_per_row', 'rows',  'l2miss_sc_meas', 'l2miss_sc_pred', 'l2ape_sc', 'variance', 'nnz_per_row_var_coeff']])
